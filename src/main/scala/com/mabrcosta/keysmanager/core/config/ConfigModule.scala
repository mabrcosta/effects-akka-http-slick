...
import com.mabrcosta.keysmanager.core.config.util.ConfigFactoryExt
...
class ConfigModule extends ScalaModule with LazyLogging {
  ...
  protected[this] def loadConfig() = {
    ...
    logger.trace(
      s"${config.root.render(ConfigRenderOptions.concise.setFormatted(true))}")
    config
  }
